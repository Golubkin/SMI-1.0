...
/*!
    ...
    Самое точное совпадение происходит при x = 0.5. Поэтому выбираем интервал от 0.5 до 2.0.
    ...
    Делается это для того, чтобы путем нескольких повторений аргумент попадал в промежуток от 0.5 до 2.0.

    Если x > 0.0 и x < 0.5, то, используя свойста логарифмов \f$\ ln(a*b) = ln(a) + ln(b) \f$, 
    приводим к промежутку от 0.5 до 2.0. С помощью такого приведения мы исходный логарифм представляем 
    в виде суммы логарифма с двойным аргументом и логарифма от 0.5. Рано или поздно этот двойной аргумент 
    придет к нужному интервалу. И сработает другое условие, где x >= 0.5 и x <= 2.0.
...
/*!
    ...
    Переменная int_x округляется вниз с помощью SMIfloor(x), а fract_x находит дробную часть.
    ...
    Реализуется на подобие как и в пункте 2. Только округление отрицательных чисел
    производится с помощью SMIceil(x). Далее представляем число в отрицательной степени 
    как один делить на тоже число только в положительной степени. И используем готовую
...
/*!
    ...
    то минимальный нижнbй предел равен нулю, а верхний предел равен самому x. Причем при x <= 1 верхний предел равен 1.
...
/*!
    ...
    Функция возвращает синус угла х (радианы). n - количество слагаемых в ряде Маклорена. 
